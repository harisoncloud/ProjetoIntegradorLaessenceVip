<?php
    ...
    class MagicToolboxOpencartModuleInstallerClass extends MagicToolboxCoreInstallerClass {
        ...
        function isModuleInstalled() {
            ...
            if(file_exists($this->dir.'/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module/magic360.module.core.class.php')) {

                if('magic360' == 'magicscroll') {
                    $contents = file_get_contents($this->dir.'/admin/controller/'.$this->modulesPath.'/magic360-opencart-module/magicscroll.module.core.class.php');
        ...
        function installFiles() {
            ...
            //if (version_compare($this->version,'2.3','>=')) {
                ...
                if ('magic360' == 'magic360') { //copy gallery files
            ...
            $directories = glob($this->dir . '/'.$admin_folder_name.'/language/*' , GLOB_ONLYDIR);
            foreach ($directories as $ldir) {
                if (!file_exists($ldir.'/'.$this->modulesPath.'/magic360.php') && file_exists($this->dir . '/'.$admin_folder_name.'/language/'.$this->englishPath.'/module/magic360.php')) {
                    if(!is_dir($ldir.'/'.$this->modulesPath)) {
                        if(mkdir($ldir.'/'.$this->modulesPath)) {
                            $this->log('CREATE DIR '.$ldir.'/'.$this->modulesPath);
                        }
                    }
                    if(copy($this->dir.'/'.$admin_folder_name.'/language/'.$this->englishPath.'/module/magic360.php', $ldir.'/'.$this->modulesPath.'/magic360.php')) {
                        $this->log('CREATE FILE '.$ldir.'/module/magic360.php');
                    }
                }
            }
            ...
            if ($this->vqmodCheck() && version_compare($this->version,'2','<')) { //use vqmod
                ...
                $this->copyFile(dirname(__FILE__).'/module/vqmod/xml/magic360_vqmod.xml', $this->dir.'/vqmod/xml/magic360_vqmod.xml');
                if('magic360' == 'magic360'){
                ...
                $this->copyFile(dirname(__FILE__).'/module/system/magic360.ocmod.xml', $this->dir.'/system/magic360.ocmod.xml');
                ...
                if(in_array('magic360',array('magiczoom','magiczoomplus','magicthumb'))) {
		...
		$replace = 'global $aFolder;'. "\n\t" . 
			  ...
			  'if (!isset($GLOBALS[\'magictoolbox\'][\'magic360\']) && !isset($GLOBALS[\'magic360_module_loaded\'])) {'. "\n\t" .
			      'include (preg_match("/components\/com_(ayelshop|aceshop|mijoshop)\/opencart\//ims",DIR_APPLICATION,$matches)?\'components/com_\'.$matches[1].\'/opencart/\':\'\').$aFolder.\'/controller/'.$this->modulesPath.'/magic360-opencart-module/module.php\';'. "\n\t" .
			  '};'. "\n\t";
		...
		$replace = '$magicArray[\'magic360\'] = true;' . "\n\t";
		$replace = $this->addComments($replace);
		...
		if (!$this->wearenotalone) {
		    ...
		    $replace = '$magicContent = magic360($magicContent,$this,\'category\',$results);' . "\n\t";
		    $replace = $this->addComments($replace);
		...
		/*if (!$this->wearenotalone) {
		    ...
		    //$replace = '$magicContent =magic360($this->render(TRUE),$this,\'manufacturers\', $results);' . "\n\t";
		    if ($version >= 2) { 
			$replace = '$magicContent = magic360($this->load->view($this->config->get(\'config_template\') . \'/template/product/manufacturer_info.tpl\', $data),$this,\'manufacturers\', $results);' . "\n\t";
		    } else {
			$replace = '$magicContent = magic360($this->render(TRUE),$this,\'manufacturers\', $results);' . "\n\t";
		    }
		    $replace = $this->addComments($replace);
		...
		$replace = '$magicContent = magic360($magicContent,$this,\'manufacturers\', $results);' . "\n\t";
		...
		/*if ($version < 2) { 
		    $pattern = '\$this->response->setOutput\(\$this->render\(TRUE\), \$this->config->get\(\'config_compression\'\)\);';
		    $replace = '$this->response->setOutput(magic360($this->render(TRUE),$this,\'search\', (isset($results) ? $results : array() )), $this->config->get(\'config_compression\'));';
		} else {
		    $pattern = '\$this->response->setOutput\(\$this->load->view\(\$this->config->get\(\'config_template\'\)\s+\.\s+\'\/template\/product\/search\.tpl\', \$data\)\);';
		    $replace = '$this->response->setOutput(magic360($this->load->view($this->config->get(\'config_template\') . \'/template/product/search.tpl\', $data),$this,\'search\',(isset($results) ? $results : array() )));	';
		}
		$c = preg_replace('/'.$pattern.'/is', $replace, $c, 2); //only first two needles replace*/
		...
		/*if ($version >= 2) { 
		    $replace = '$magicContent = magic360($magicContent,$this,\'search\', $results);' . "\n\t";
		} else {*/
		    $replace = '$magicContent = magic360($magicContent,$this,\'search\', $results);' . "\n\t";
		/*}*/
		...
		/*if ($version < 2) { */
		    $pattern = '\$this->render\(\);';
		    $replace = 'global $aFolder; include($aFolder.\'/controller/'.$this->modulesPath.'/magic360-opencart-module/boxes.inc\');';
		    $replace = $this->addComments($replace);
		    $replace = '$this->render();'."\n\t".$replace;
		/*} else {
		  if (!$this->wearenotalone) {
			...
			$replace = 'global $aFolder; include($aFolder.\'/controller/module/magic360-opencart-module/boxes.inc\');  ';
			$replace = $this->addComments($replace);
			$replace = '$contents = $1;'."\n\t". $replace .'return $contents;'."\n\t";
		    } else {
			$pattern = 'return\s+\$contents;';
			
			$replace = "\n\t".'global $aFolder; include($aFolder.\'/controller/module/magic360-opencart-module/boxes.inc\');';
			$replace = $this->addComments($replace);
...
//		$c = preg_replace('/'.$pattern.'/is', $replace."\n\t".'$1', $c, 1); //only first needle replace
		...
		/*if ($version < 2) { */
		    ...
		    $replace = 'if(version_compare(VERSION, \'1.4.9\', \'<\')) {' . "\n\t\t\t" .
				  '$this->output = magic360($this->output,$this,\'latest_home_category\',$this->model_catalog_product->getLatestProducts(8));' . "\n\t\t" .
			      '}' . "\n\t\t";
		    $replace = $this->addComments($replace);
		    $replace = '$this->render();' . "\n\t\t" . $replace;
		...
		/*if ($version < 2) {*/
		    ...
		    $replace =  '$magic360_config = $this->config->get(\'magic360_settings\');'. "\n\t\t\t" .
				'if($magic360_config[\'magic360_status\'] != 0) { '. "\n\t\t\t" .
				    '$tool  = magic360_load_core_class($this);' . "\n\t\t\t" .
				    'if( magic360_use_effect_on($tool)) {' . "\n\t\t\t\t" .
					'$magicArray[\'magic360\'] = true;' . "\n\t\t\t" .
				    '}' . "\n\t\t" .
			    '}';
		/*} else {
		    if (!$this->wearenotalone) {
			...
			$replace =  '$1 '. "\n\t\t\t" .' $magic360_config= $this->config->get(\'magic360_settings\');'. "\n\t\t\t" .
				'if($magic360_config[\'magic360_status\'] != 0) {' . "\n\t\t\t" .
				      '$tool  = magic360_load_core_class($this);' . "\n\t\t\t" .
				      'if( magic360_use_effect_on($tool)) {' . "\n\t\t\t\t" .
					  '$magicArray[\'magic360\'] = true;' . "\n\t\t\t" .
        ...
        function restoreStep_installFiles() {
            ...
            $files_to_remove=array('/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360.php',
                                    '/'.$admin_folder_name.'/view/template/'.$this->modulesPath.'/magic360.tpl');
		...
		if (version_compare($this->version,'2','<')) {
		    $files_to_remove[] = '/vqmod/xml/magic360_vqmod.xml';
		    if('magic360' == 'magic360'){
		    ...
		    $files_to_remove[] = '/system/magic360.ocmod.xml';
		    if('magic360' == 'magic360'){
            ...
            $directories = glob($this->dir . '/'.$admin_folder_name.'/language/*' , GLOB_ONLYDIR);
            foreach ($directories as $ldir) {
                if (file_exists($ldir.'/'.$this->modulesPath.'/magic360.php')) {
                    $files_to_remove[] =  str_replace($this->dir,'',$ldir.'/module/magic360.php');
                }
            }

            $this->copyJsCss('remove');
            $this->removeFiles($files_to_remove);
            $this->removeDir($this->dir . '/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module');
            $this->removeDir($this->dir . '/'.$admin_folder_name.'/view/image/magic360-opencart-module');
            return true;
        }
        ...
        function upgrade($files) {
            ...
            $path = $this->dir . '/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module/';
        ...
        function copyJsCss($action = false) {
            ...
            if (file_exists($this->dir . '/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module/graphics')) {
		$this->createDirRecursive($this->dir.'/catalog/view/css/graphics');
		$this->copyDir ($this->dir . '/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module/graphics',
				$this->dir.'/catalog/view/css/graphics');
	    }
            
            if (file_exists($this->dir . '/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module/fonts')) {
		$this->createDirRecursive($this->dir.'/catalog/view/css/fonts');
		$this->copyDir ($this->dir . '/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module/fonts',
				$this->dir.'/catalog/view/css/fonts');
            }
            
            if (file_exists($this->dir . '/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module/themes')) {
		$this->createDirRecursive($this->dir.'/catalog/view/css/themes');
		$this->copyDir ($this->dir . '/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module/themes',
				$this->dir.'/catalog/view/css/themes');
            }
            
            $dirHandler = opendir($this->dir . '/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module');
            while (false !== ($entry = readdir($dirHandler))) {
                if (preg_match('/\.(js|css|swf)/is',strtolower($entry),$copyMatches)) {
                    switch ($copyMatches[1]) {
                        case 'js': { 
                                      if ($action == 'install') {
                                          $this->copyFile($this->dir.'/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module/'.$entry,
			...
			case 'swf': { 
                                      if ($action == 'install') {
                                          $this->copyFile($this->dir.'/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module/'.$entry,
							  $this->dir.'/catalog/view/javascript/'.$entry);
					  if (file_exists($this->dir . '/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module/loader.gif')) {
					      $this->copyFile($this->dir.'/'.$admin_folder_name.'/controller/'.$this->modulesPath.'/magic360-opencart-module/loader.gif',
        ...
        function addComments ($content) {
            $content = '/* Magictoolbox magic360 module BEGIN */'."\n\t".$content ."\n\t".'/* Magictoolbox magic360 module END */'. "\n\t";
            return $content;
        }

        function removeModuleFromFiles ($files) {
            $pattern = '\/\*\s+Magictoolbox\s+magic360\s+module\s+BEGIN\s+\*\/.*?\/\*\s+Magictoolbox\s+magic360\s+module\s+END\s+\*\/';
            if (is_array($files)) {
