	...
	<h3 class="settingsTitle">Module status</h3>
        ...
        <table class="params" cellspacing="0">
            <tr class="last">
                ...
                <td width="70%">
                    <select name="magic360_status">
                    <?php if ($this->config->get('magic360_status')) { ?>
                            ...
                            switch($s["type"]) {
                                case "array":
                                        ...
                                        foreach($s["values"] as $p) {
                                            ...
                                            $rButtons[strtolower($p)] = '<label><input type="radio" value="'.$p.'"'. ($s["value"]==$p?"checked=\"checked\"":"").' name="'.$profile.'_'.$s['id'].'" id="magic360settings'. ucwords(strtolower($name)).$p.'">';
                                            $pName = ucwords($p);
                                            if(strtolower($p) == "yes")
                                ...
                                default:
                                    ...
                                    if (!isset($s["value"])) $s["value"] = $s["default"];
                                    $html .= '<input '.$width.' type="text" name="'.$profile.'_'.$s['id'].'" id="magic360settings'. ucwords(strtolower($name)).'" value="'.$s["value"].'" />';
                                    break;
                            }
